...
IF EXISTS (SELECT * FROM  dbo.sysobjects WHERE id = object_id(N'[dbo].[sp_asignarCromosoma]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE sp_asignarCromosoma    
GO
...
BEGIN
		...
		IF EXISTS (SELECT * FROM SM_CROMOSOMA WHERE NUMEROCROMOSOMA = @NUMEROCROMOSOMA)
		BEGIN
...
sp_columns SM_CROMOSOMA
--falta ejecutar y hacer la llamada.
...
--ELIMINAR POR CADA EJECUCION DE PROGRAMA.....
