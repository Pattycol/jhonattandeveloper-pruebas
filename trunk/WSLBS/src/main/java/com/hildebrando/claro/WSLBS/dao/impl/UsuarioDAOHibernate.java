...
public class UsuarioDAOHibernate extends DAO<Usuario> implements UsuarioDAO{
	...
	public List<Usuario> getUsuariosPorArea(Area area){
		//String sql="SELECT u FROM Usuario u WHERE u.estado<>:estado AND :area in elements(u.areas) ORDER BY u.numero";
		String sql="SELECT u FROM Usuario u WHERE :area in elements(u.areas) ORDER BY u.numero";
		Query q=em.createQuery(sql);
    ...
    public Usuario buscarPorNumero(String numero) {
        String sql = "SELECT u FROM Usuario u WHERE u.numero=:numero";
		Query q=em.createQuery(sql);
		q.setParameter("numero",numero);
        // q.setParameter("estado", Constantes.ESTADO_ACTIVO);
		try{
